...
未完成或未验证事项：
  1. 定时触发未验证 —— GitHub Actions 的 schedule（每日 UTC 22:00 = 北京 06:00）已配置好，
     且手动触发（workflow_dispatch）已在 GitHub 上跑通（11 步全绿、Pages 部署成功），
     但交付当天尚未到达下一个 cron 时刻，cron 按点自动触发这一条还没有实际发生过。
  2. 有意未做（非遗漏）：
     · AI 摘要生成 —— 砍单顺序第 1 位，避免引入外部 API 依赖与密钥；
     · 抓取文章页 og:image —— 需要 234 次额外请求，且带来图片体积 / 防盗链 / 反爬三类问题；
     · 源级超时预算与熔断 —— 当前只有 Google AI Blog 一个源受影响；
     · 中文纯标题的跨源事件聚合 —— 宁可漏聚，也不虚报「多来源报道」。
  3. 已知限制：
     · 配图率 27.4%（234 条中 64 条）是「不额外请求文章页」约束下的天花板；
     · 近 7 天源成功率 0.625 被 Google AI Blog 拖低 —— 中国大陆直连 blog.google 会 TLS 超时，
       该源失败是间歇性的，失败被如实记录且不影响其它 15 个源。
```
