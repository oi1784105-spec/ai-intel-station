...
未完成或未验证事项：
  ...
  3. 已知限制：
     · 配图率 25.7%（389 条中 100 条）是「不额外请求文章页」约束下的天花板；
     · 近 7 天运行成功率 0.65（口径：整轮无失败源的轮次占比，13/20；按源次计则为 313/320 = 97.8%）——
...
| 演示地址 | <https://oi1784105-spec.github.io/ai-intel-station/> | 线上实测 HTTP 200、620,486 字节、`sha256:f5554500b8adbbe9`，与本地 `dist/index.html` **字节级完全一致**（已是第三版，含手机端版式修复） |
