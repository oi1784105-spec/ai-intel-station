...
样例断言：`清除后回到全量 [234 / 234]`、`搜索生效且为子集`、
`所有缩略图盒子尺寸一致 [96x72]`、`没有标题渲染超过两行 [44px ≤ 47px]`、
`没有摘要渲染超过两行 [42px ≤ 47px]`、`主题可切换 [dark → light]`。

另做了一次针对性的渲染核对：**234 张卡片的时间文案各出现恰好 1 次**，重点区 5 条各 1 次，
...
# → 已生成 dist/index.html（382.4 KB / 391,559 字节）
...
| **代理 / 境外出口切换下的稳定性** | `google-ai` 的成败随网络状态变化 | 失败隔离已由离线确定性实验覆盖；两种出口各实测一次（本地直连失败、云端成功） |
| **大规模源下线时的观感** | 未构造该场景 | 无图源占多数时用源色占位块兜底（已实测 234 张卡片） |
...
| **抓取文章页 `og:image`** | 需要 234 次额外请求，并引入图片体积、防盗链、被反爬三类问题。当前配图率 27.4% 是现状约束下的天花板 |
| **源级超时预算 / 熔断** | 当前只有一个源受影响（约 +65 秒/轮）。多源出现该情况时应引入 |
| **中文纯标题的事件聚合** | 见 README 已知限制第 3 条：跨源互证目前依赖拉丁专名。宁可漏聚，也不虚报「多来源报道」 |
| **实时搜索 / 分页** | 静态站形态的固有限制，数据全量内联（234 条约 382 KB） |
...
| 文件 / 路径 | 内容 | 提交 |
| --- | --- | --- |
| `data/runs.json` | **16 次运行的完整记录**（逐源状态、错误、计数、耗时） | ✅ 已提交 |
| `data/update-status.json` | 最近一轮的整轮结论与逐源健康度 | ✅ 已提交 |
| `data/news.json` | 234 条归一化条目（含 `published_at` / `first_seen_at` / 配图 / 聚合） | ✅ 已提交 |
...
| `dist/index.html` | 单文件交付物（数据已内联，391,559 字节） | ✅ 已提交 |
