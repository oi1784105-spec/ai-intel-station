...
## 在线版本沿革（初版 → 第二版 → 第三版 → 第四版）

线上演示地址跑的是**第四版**。四版都留了截图，便于对照。
...
tools/
  ...
  verify_ui.py       无头 Chrome 真实渲染 + 真实点击校验（119 项断言，含 360px / 390px 手机段）
...
前端另有一套独立校验（`tools/verify_ui.py`，119 项）：无头 Chrome 真实渲染后**真的去点**
筛选标签、来源、语言徽标、主题按钮与「清除全部」，断言实际筛出的集合是否正确，
而不是只看静态 DOM 有没有生成元素。第二版为此新增三条护栏：重点区右栏不留空洞、
未读正文对比度达标、变淡只属于已读态。第三版又把手机窄屏钉成 10 条断言
（390px 视口下不横向滚动、可点控件触控高度 ≥32px、报头不抢屏等，逐条对照见
`VERIFICATION.md` 1.6.1）—— 全部改动只落在 `max-width:1023px` / `640px` 两段，
...
1. **Google AI Blog 会间歇性抓取失败（已定位到地址层，未修）**。实测 `blog.google` 的 4 个 A 记录里
   ...
   的两极表现（`data/runs.json` 里 20 轮：该源 7 轮成功、7 轮失败、6 轮无新条目，失败那次固定耗掉约 65 秒；
   全仓源级成功率 313/320 = 97.8%，且全部失败都来自这一个源）。
...
6. **配图只用 feed 载荷里自带的，不额外抓文章页**。提取范围限于 feed 已经下载下来的
   ...
   因此**不产生任何额外请求**。实测 16 个源里 9 个提供配图，覆盖率 25.7%（389 条中 100 条），
   其余卡片显示源色占位块 —— 这是该约束下的天花板，不是实现缺陷。
   要把覆盖率提上去只能逐篇抓 `og:image`，那会引入 389 次额外请求、图片体积与防盗链三类问题，
