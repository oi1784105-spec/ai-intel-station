...
| 一个信源反复抓取失败，原因不明 | 逐层拆 DNS / TCP / TLS，定位到 `blog.google` 的可用地址里有一半不可用（详见「已知限制」第 1 条） | 截至本次提交的 25 轮里该源 8 轮成功（秒级）/ 7 轮失败（每次约 65 秒）/ 10 轮无新条目，7 次失败**全部**来自它；失败被如实上报，不影响其余 15 个源 |
...
（「电脑端仍是 12 栏网格」「电脑端不显示控制条」），总数 **119 → 161**；第六版再补 5 条 → 166；第六版二稿再补 4 条 → **170**（170 / 170 通过）。
...
| 自检发现 | 做法 |
| --- | --- |
| 434 条里 **94 条**源站 feed 没给摘要，卡片只剩标题 —— 不满足 §03-02「标题 + 简短摘要**或来源简介** + 来源 + 时间 + 原文链接」 | 按来源 id 加一句**来源简介**兜底（只写订阅配置里能核实的属性：语言 / 类型 / 是否官方 / 是否做相关性过滤），一律以 `来源简介：` 开头，和 `摘要：来源 feed 原文` 显式区分 —— 它是「这个源是什么」的说明，不是对文章内容的概括，**不补造事实** |
...
| 换一批数据后，窄屏轮播**第 2 屏只填 46% 高度**（该条重点既没配图、源站也没给摘要，一屏只剩标题 + 一行来源简介）→ v5 那条「每屏内容占框高 ≥75%」的断言当场报红 | **二稿改法**：一开始给这类条目补了「源色占位块」，但读者反馈那块占位在窄屏里像在报警（上面写着「本条来源未提供配图」）—— 于是**撤掉占位块与全部「无配图」提示**，改走**纯文字**：余量交给「摘要 / 来源简介」那一行自己撑开、来源简介在行内垂直居中，卡片成「元信息 / 标题 + 简介 / 标签」三段分布；各屏 **90 / 95 / 96 / 96 / 96 %**（360px）、**89 / 95 / 96 / 96 / 96 %**（390px），并补两条断言钉住「窄屏不出现任何『无配图』字样」（免得改回去） |

自检逐条结论（含缺少日期 / 长标题 / 无图 / 搜索无结果 / 干净环境跑通 / 抽查原文）见
[`VERIFICATION.md` §1.10](VERIFICATION.md)。

> 口径提醒：第六版**电脑端也会变**（434 条里 94 条、默认视图里 52 张卡片会多一行「来源简介」小字），
> 所以「桌面截图逐字节不变」只适用于**第四版 / 第五版**那两次改造的取证，不代表第六版。
> 第六版改的是**内容兜底与文案**（二稿另加：窄屏里没配图的重点条目改走纯文字，只动 `max-width:1023px` 段）：
...
tools/
  ...
  verify_ui.py       无头 Chrome 真实渲染 + 真实点击校验（170 项断言，含 360px / 390px 手机段）
...
前端另有一套独立校验（`tools/verify_ui.py`，170 项）：无头 Chrome 真实渲染后**真的去点**
...
（桌面 3 + 手机段 2）→ 166；第六版二稿再补手机段 2 项 × 360/390 两档 → **总数 170 项**（见 1.10）—— 全部改动只落在 `max-width:1023px` / `640px` 两段，
...
1. **Google AI Blog 会间歇性抓取失败（已定位到地址层，未修）**。实测 `blog.google` 的 4 个 A 记录里
   ...
   的两极表现（`data/runs.json` 里 25 轮：该源 8 轮成功、7 轮失败、10 轮无新条目，失败那次固定耗掉约 65 秒；
   全仓源级成功率 393/400 = 98.2%，且全部失败都来自这一个源）。
...
6. **配图只用 feed 载荷里自带的，不额外抓文章页**。提取范围限于 feed 已经下载下来的
   ...
   因此**不产生任何额外请求**。实测 16 个源里 9 个提供配图，覆盖率 25.3%（434 条中 110 条），
   其余卡片显示源色占位块 —— 这是该约束下的天花板，不是实现缺陷。
   要把覆盖率提上去只能逐篇抓 `og:image`，那会引入 434 次额外请求、图片体积与防盗链三类问题，
