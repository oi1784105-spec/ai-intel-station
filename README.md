...
| 一个信源反复抓取失败，原因不明 | 逐层拆 DNS / TCP / TLS，定位到 `blog.google` 的可用地址里有一半不可用（详见「已知限制」第 1 条） | 19 轮里该源 12 轮成功（1 秒级）/ 7 轮失败（每次约 65 秒），7 次失败**全部**来自它；失败被如实上报，不影响其余 15 个源 |
...
1. **Google AI Blog 会间歇性抓取失败（已定位到地址层，未修）**。实测 `blog.google` 的 4 个 A 记录里
   ...
   的两极表现（`data/runs.json` 里 19 轮：该源 12 轮成功、7 轮失败，失败那次固定耗掉约 65 秒；
   全仓源级成功率 297/304 = 97.7%，且全部失败都来自这一个源）。
...
6. **配图只用 feed 载荷里自带的，不额外抓文章页**。提取范围限于 feed 已经下载下来的
   ...
   因此**不产生任何额外请求**。实测 16 个源里 9 个提供配图，覆盖率 25.4%（299 条中 76 条），
   其余卡片显示源色占位块 —— 这是该约束下的天花板，不是实现缺陷。
   要把覆盖率提上去只能逐篇抓 `og:image`，那会引入 299 次额外请求、图片体积与防盗链三类问题，
