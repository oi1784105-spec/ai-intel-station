...
## 在线版本沿革（初版 → 第二版 → 第三版 → 第四版 → 第五版）

线上演示地址跑的是**第五版**。五版都留了截图，便于对照。
...
| 一个信源反复抓取失败，原因不明 | 逐层拆 DNS / TCP / TLS，定位到 `blog.google` 的可用地址里有一半不可用（详见「已知限制」第 1 条） | 截至本次提交的 20 轮里该源 7 轮成功（秒级）/ 7 轮失败（每次约 65 秒）/ 6 轮无新条目，7 次失败**全部**来自它；失败被如实上报，不影响其余 15 个源 |
...
| 每屏等高，但不能空出半张白卡 | 屏高由最高那屏决定，于是让矮的屏**自己吃掉余量**：摘要 `flex:1 1 auto`（末尾仍是省略号）、带缩略图的屏让缩略图竖着撑满、标签行 `margin-top:auto` 钉底 | 各屏内容占框高 **90 / 97 / 97 / 97 / 97 %**（改前纯文字那屏只有 **48%**，视觉上像破图） |
| 头条那屏本来最高（带 16:9 大图） | 窄屏收起头条的大图（桌面照旧显示大图；这条情报在下方「全部情报」卡片里仍带自己的配图），标题 3 行、正文 4 行，省略号收 | 整条高度 **528 → 482px**；没有一屏的内容被硬切 |

只动 `web/index.html` 的窄屏两段。`tools/verify_ui.py` 手机段新增 **20 条**、电脑段新增 **2 条**
（「电脑端仍是 12 栏网格」「电脑端不显示控制条」），总数 **119 → 161**（161 / 161 通过）。
...
tools/
  ...
  verify_ui.py       无头 Chrome 真实渲染 + 真实点击校验（161 项断言，含 360px / 390px 手机段）
...
前端另有一套独立校验（`tools/verify_ui.py`，161 项）：无头 Chrome 真实渲染后**真的去点**
...
电脑段再加 2 条确认轮播不在桌面留痕，**总数 161 项**（见 1.6.3）—— 全部改动只落在 `max-width:1023px` / `640px` 两段，
...
1. **Google AI Blog 会间歇性抓取失败（已定位到地址层，未修）**。实测 `blog.google` 的 4 个 A 记录里
   ...
   的两极表现（`data/runs.json` 里 21 轮：该源 7 轮成功、7 轮失败、7 轮无新条目，失败那次固定耗掉约 65 秒；
   全仓源级成功率 329/336 = 97.9%，且全部失败都来自这一个源）。
...
6. **配图只用 feed 载荷里自带的，不额外抓文章页**。提取范围限于 feed 已经下载下来的
   ...
   因此**不产生任何额外请求**。实测 16 个源里 9 个提供配图，覆盖率 26.1%（395 条中 103 条），
   其余卡片显示源色占位块 —— 这是该约束下的天花板，不是实现缺陷。
   要把覆盖率提上去只能逐篇抓 `og:image`，那会引入 395 次额外请求、图片体积与防盗链三类问题，
