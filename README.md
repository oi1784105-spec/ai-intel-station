...
线上演示地址跑的是**第二版**（已替代初版）。两版都留了截图，便于对照。
...
| 一个信源反复抓取失败，原因不明 | 逐层拆 DNS / TCP / TLS，定位到 `blog.google` 的可用地址里有一半不可用（详见「已知限制」第 1 条） | 18 轮里该源 11 轮成功（1 秒级）/ 7 轮失败（每次约 65 秒），7 次失败**全部**来自它；失败被如实上报，不影响其余 15 个源 |
| 「重要度评分」后面用顿号，读起来像并列 | 改为冒号（它后面跟的是「用什么算的」） | 套件里新增一条断言把整句字面钉住，改动不会静默回退 |

改动只落在 `web/index.html` 与其构建产物 `dist/index.html`；采集管线未改。
...
tools/
  ...
  verify_ui.py       无头 Chrome 真实渲染 + 真实点击校验（89 项断言）
...
前端另有一套独立校验（`tools/verify_ui.py`，89 项）：无头 Chrome 真实渲染后**真的去点**
...
1. **Google AI Blog 会间歇性抓取失败（已定位到地址层，未修）**。实测 `blog.google` 的 4 个 A 记录里
   ...
   的两极表现（`data/runs.json` 里 18 轮：该源 11 轮成功、7 轮失败，失败那次固定耗掉约 65 秒；
   全仓源级成功率 281/288 = 97.6%，且全部失败都来自这一个源）。
...
3. **抓取全部失败时 `meta.json` 会被写成空（已知缺陷，未修）**。`news.json` 有仓储保底不会损坏，
   ...
   （14 个源 HTTP 200，库中 234 → 266 条）。修法（失败时不要覆盖 `meta.json`）留待下一轮，`pipeline/` 本轮有意不动。
...
6. **配图只用 feed 载荷里自带的，不额外抓文章页**。提取范围限于 feed 已经下载下来的
   ...
   因此**不产生任何额外请求**。实测 16 个源里 9 个提供配图，覆盖率 27.5%（276 条中 76 条），
   其余卡片显示源色占位块 —— 这是该约束下的天花板，不是实现缺陷。
   要把覆盖率提上去只能逐篇抓 `og:image`，那会引入 276 次额外请求、图片体积与防盗链三类问题，
