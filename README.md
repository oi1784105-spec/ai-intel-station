...
- **在线版**：仓库启用 GitHub Pages 后为 `https://<用户名>.github.io/<仓库名>/`
...
| 开发说明 | [`DEVLOG.md`](DEVLOG.md) | 实际投入时间（**7.1h**）、主要开发工具与 AI 使用情况、**两个关键决策**、问题定位与返工过程 |
...
tools/
  ...
  verify_ui.py       无头 Chrome 真实渲染 + 真实点击校验（39 项断言）
...
| 2 | **摘要是纯导航链接** | InfoQ **20/20** 条摘要都是 `点击查看原文>`（平均 7 字符）；全部源合计 279 条中 35 条（12.5%）摘要为空或极短 | `clean_summary()`：转纯文本 → 剥样板文案 → 低于 `SUMMARY_MIN_CHARS` 即置空。前端宁可不渲染摘要，也不渲染一句「点击查看原文>」 |
...
前端另有一套独立校验（`tools/verify_ui.py`，39 项）：无头 Chrome 真实渲染后**真的去点**
...
1. **Google AI Blog 在中国大陆直连不可达**。实测 `blog.google` / `google.com` 的 TLS 握手 25 秒无响应，
   需代理。云端（GitHub Actions，出口在境外）正常，本地大陆环境该源会报 `failed(timeout)`
   —— 这是**正确行为**：失败被如实报告，且不影响其它 15 个源。配置项 `needs_overseas_network` 已标注。
2. **一个挂死的源会给整轮增加约 65 秒**（3 次尝试 × 20 秒超时 + 退避）。当前只有 Google 这一个源受影响；
   若将来多个源变成这样，应引入源级超时预算或熔断。
3. **跨源聚合依赖拉丁专名**。中文源与英文源能互证是因为模型名/公司名是拉丁文（`GPT-6`、`Mistral`）。
   纯中文标题之间的事件聚合只能靠标题相似度，召回率有限。
4. **聚合会有漏聚**。为了精度牺牲召回：要求 ≥2 个共享稀有实体。宁可漏聚，也不虚报「多来源报道」。
5. **配图只用 feed 载荷里自带的，不额外抓文章页**。提取范围限于 feed 已经下载下来的
   `media:content` / `media:thumbnail` / `enclosure` 与正文 HTML 的首张 `<img>`，
   因此**不产生任何额外请求**。实测 16 个源里 8 个提供配图，覆盖率约 21%（231 条中 49 条），
   其余卡片显示源色占位块 —— 这是该约束下的天花板，不是实现缺陷。
   要把覆盖率提上去只能逐篇抓 `og:image`，那会引入 231 次额外请求、图片体积与防盗链三类问题，
   有意不做。外链图片可能被 CDN 按 Referer 拒绝或已失效，前端备有 `onerror` 回退占位块。
6. **摘要不是 AI 生成的**。只做清洗与截断，不做摘要生成（不引入 LLM 调用，保持零成本与零密钥）。
7. **`data/` 必须提交进仓库**。它既是站点数据源，也是「定时任务每天真的在跑」的唯一证据。
8. **修正解析配置后需要 `--rebuild`**。合并更新是单调的（不覆盖已有字段），因此已入库的
   错误值（如更正前的时区）不会就地改写。这是有意为之：单调更新是幂等性的前提。

## 时间预算与砍单顺序

8 小时预算，超出时按下表顺序砍，**先砍的排在最前面**：
